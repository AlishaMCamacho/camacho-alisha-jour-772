...
**Answer**
id = text 
seats = so, both "chr" and "dbl" are both text themselves, but "chr" refers to text, and "dbl" refers to numbers.
...
**Updated prompt** In the space below, write the name of the course from the first row of the "title" column
...
**Answer**
"umd_courses" is the name of the data set
the operator, %>% , is telling R to group by term
summarise tells R to create a new column called "count_classes"
"= n()" is telling R to count the number of observations in the group, which in this case is "term."
...
The data is now arranged in descending order via the term column/variable.  
...
The code, "group_by(term, department)"added the department variable back in. 
The code, "arrange(term, department) told R to arrange the data by term first, and then by department. 
...
- The variable "department" was removed
- The new column, "count classes" now contains the number of observations in the group "total_seats". The data is then arranged in descending order by total seats. 
...
Three additional columns/variables were added: "total_seats," "mean_seats," and "median_seats." 

The top terms all start with "202" and end with "08," and have more than 21,7314 seats. The mean number of seats is also higher than the median number in most cases. 
...
The data is now grouped by department. I adjusted the code to match the code from the textbook, as I wasn't seeing the min/max prior. With the alteration, I can now see the minimum number of seats is zero, and the max number of seats.  
...
1.  I would want to add a column to see what how full or not classes are on average by percentage, and how many classes are offered per academic year, and map over time to find patterns about popular classes and less popular classes.

2.  I would be curious to learn how many classes are offered by department

3.  This would require more data, but I would be interested to learn how much funding each department has, and compare that with the number of classes offered and seats filled. 
