...
Q - What does the first line of the red Warning message that prints out when you load the data say?
A - One or more parsing issues, call `problems()` on your data frame for details, e.g.:
  dat <- vroom(...)
  problems(dat)Rows: 373564 Columns: 8── Column specification 
  
Q - What happens when you follow its instructions?

A table is produced with all the parsing problems, although I am not sure how to interpret the table. 
...
No parsing errors this time. But why not? 
...
Q- What data type is the "Vendor Name" field? 
A - Text

Q- What data type is the "Amount" field? 
A- Numbers

Q-What data type is the "Date" column?
A- Date & Time (stored as characters)
...
Q- How many zip codes do not have five characters? 
A- 549

Q-What do you think explains those records? 
A- for the numerical records, possibly human error. For the character records, they are likely outside of the United States. Some appear to be Canadian zip codes. 

```{r}
payments |>
#  clean_names()
#  as.numeric (`Vendor Zip`)
  group_by(`Vendor Zip`) |>
...
Q: Run the following codeblock and describe below what each line is doing.
A:
"md_grants_loans |> " is the dataset we are looking at, and we are telling it to...
"group_by(`Fiscal Year`, Category) |>" group the fiscal years together by cateogories. So, all the 2010 grants are grouped together in one row, as are the 2010 loans. 
"summarize(count = n()) |>" tells R to count all the values in the same row, that were group together, and summarize them into one value. 
"arrange(`Fiscal Year`)" is telling R to sort the data is ascending order via fiscal year. 
...
**Answer**
I see that the Land Preservation Trust got a miserly grant. There appears to be a large range of amounts given to grantees. 


```{r}
md_grants_loans |> 
  arrange(Amount) |> 
  select(Grantor, Grantee, Category, Amount)
```
...
**Answer**
The headers are now all in lowercase and headers with more than one word are underscored. Example, "fiscal_year."
...
cleaned_md_grants_loans <- md_grants_loans |>
*creates a new data set, saved as "cleaned_md_grants_leans, to reflect the functions below*

clean_names() |> 
* standarizes the headers. all lower case and underscapres.*

rename(source = grantor) |> 
*changes the name of the column from grantor to source*  
  
mutate(source = str_to_upper(source), grantee = str_to_upper(grantee), description = str_to_upper(description))

*creates new columns, where all the characters are now all caps.*
...
**Answer**
A new column, zip5, shows the zip codes with the first 5 numerical values.
...
It is downtown Baltimore, including the inner Harbor. 

**how can I add the other columns back in? I want to see the grantees, description and years awarded.** 
