...
```{r}
alldata |> 
  mutate(change = ((Population2010-Population2000)/Population2000)*100) |> 
  arrange(desc(change))
```
...
**Answer**
Median Household By County in Maryland
...
Which jurisdictions saw the largest increases in median income? 

**Answer**
Baltimore city, Allegancy County, Calvert County

Did any counties see their median incomes fall from 2010-2021?'
...
**Task** Let's get ZCTA household income data from the ACS using tidycensus for each zip code in `md_high_schools`. Run the following codeblock to retrieve household income data from 2019 - the latest year for which we have it for all Maryland ZCTAs - first, then join them to `md_high_schools` using the GEOID column. We'll also create min and max values and drop any rows where we don't have income estimates. 

What's one question we could ask of this data now that we have enrollment percentages and household income for the high school's zip code? What might make the answer harder to interpret?

**Answer**
Question: could enrollment percentages correlate with household incomes? *I specifically said correlate, because there are too factors to say for certain whether household income affects enrollments. If there is a trend, it is cause to look into further, not draw any conclusions.  
...
High schools that have 60% or more graduates immediately enrolling in college have an average household income of $111,448.3, and high schools with less than 40% of graduates immediately enrolling in college have an average household income of $69,252.24	
...
**Answer**
How many high schools with 60% or more graduates immediately enrolling in college vs. less than 40% are there per each local school system? How does this compare to zip code? 
