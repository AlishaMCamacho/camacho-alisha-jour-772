...
The new variable, "selected_journalism_courses" lets us focus our search within that data, as opposed to all of the umd courses offered. It allows us to zoom in and focus on an aspect of the data we want to look at. 
...
**Answer:**
The function mutate transformed the expenditure data from characters to numerals (date), and the lubridate function "mdy" changed the layout to a date format. 

**Question**
The code says "mdy" but the output is "ymd." why did this happen? 
...
"mutate" created a new column called "month." the "floor_data" function rounds the values down to the first say of the month, and then "group_by(month) groups the data together by month, and summarizes them with "summarize(). So expenditures from June 1st and June 30th are both represented in "June."  

**disclaimer** Chat CPT helped me unpack this one 
...
**Task** Let's add a new column based on an existing column. Run the following code to create a new column called `total_votes` based on the votes cast for each candidate, and then to create a new column called 'total_Moore'
...
**Answer**
* I added 'harding' into the code*

The code is adding the total votes in each column listed (cox, moore, etc.), and then dividal that total by the sum of moore's votes to get a percentage (in decimal form), and created a new column "pct_moore."
...
We changed the "pct_moore" from decimals to percentages, and then arranged that column in descending order to show total votes toward Moore via county. 
...
It depends on what you are looking for. Changing it to ascending order shows you which counties had the least votes for Moore, in order. 
...
- str_to_lower() converts all the characters to lowercase
- str_to_title() capitalizes the first character of each word
- str_trim() removes leading and trailing whitespaces


Question submitted to ChatGPT: "What are other functions in tidyverse are similar to "str_to_upper" in RStudio?"

*this would have been SO NICE for CRM data cleanup!!!! 
...
It creates a new column, "state," with the post office state abbreviations. It does this by telling R to detect an address with the state name spelled out through the function "state = case_when(). The function tells R to replace the state with the abbreviation in the new column, or to use NA when a match isn't found. 
...
Yes, it does. We asked R to group the rows together by state (so all of the rows with MD were lumped together as one sum) and then we asked R to arrange via descending order. 
...
1. I'd like to add a new column (mutate) that summarizes the payee-type by category to see who/what the money is ultimately going to. 

2. I'd want to help ensure all the addresses are using the same format, so choosing "street" or "St." or "st." etc. 

3.  I'd like to group/summarize the expense purpose column by type, and then organize by descending order to see where most of the money is going toward. 