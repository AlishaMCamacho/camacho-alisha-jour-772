...
https://datawrapper.dwcdn.net/a0aW0/1
...
md_gov_county <- read_csv("data/md_gov_county.csv") |> 
  ...
  mutate(fips_code_chr = as.character(fips_code))
...
#disclaimer: I used ChatGPT to create the column total_votes, and learned more about the function "rowwise" which seems really useful
https://chat.openai.com/share/a738b0eb-4895-4cb8-8be0-4a79c59f1c66
  
```{r}
# calculate the total number of votes and Moore's percentage

md_gov_county_2 <- md_gov_county |>
  rowwise() |> 
  mutate(total_votes = sum(cox + moore + lashar + wallace + harding)) |> 
...
```{r}
#save dataframe back to the md_gov_county dataframe

write.csv(md_gov_county_2, file = "md_gov_county_2.csv")
...
Datawrapper zooms in so you only see the state of Maryland. 

##

But it's a little rough visually, so let's clean that up.

**Task** Look for the "Show color legend" label and add a caption for the legend, which is the horizontal bar under the title. It represents the extent of the data from smallest number of loans to largest. 

- Then click on the "Annotate" tab to add a :

• title | should represent the headline
• description | longer phrase that tells people what they are looking at
• data source
• byline
• I am adding an alt description as well. I think it is important we learn how to properly write these for publication and get into the habit, to be inclusive and ADA compliant level II, which is also typically a requirement for any nonprofit org. 
...
When hovering the map, the tooltips now show the county name and percent of votes wes moore received. As an aside, I  changed the county names to all lowercase, to get into the habit of cleaning the data first while in R Studio. Moving forward, I would reformat the data frames as a last step before exporting to .csv, now that I understand the sequence of events to prepare this for a map.  


**Task** Add a note that says that this data represents votes counted through Nov. 10 and an alternative description for screen readers. This should be a simple description of what your map shows. 

Experiment with the "Show labels" button, choosing the "by column" option for Type to display some or all of the county names.

Ok, that looks better. Let's publish!

**answer**
I respectfully disagree lol. I removed the labels, as it looked cluttered. I think the hovering of the data is more appropriate in this case, since datawrapper doesn't let you manually adjust the placement of labels. While this is much easier to use compared to ARC (and free) this would be a disadvantage but overall I think just so much more user friendly and efficient.  It is too bad that it doesnt let the user select whether they want the labels or not, as I think that would be the best compromise when placed on a live site. 

I do love that datawrapper has  a color blind check - that is great. 
...
https://www.datawrapper.de/_/aq9oG/
