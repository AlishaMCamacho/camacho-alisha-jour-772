...
**Answer**
I see that the .shp file is formatted for geographical shapes: zip code in full and abbreviated format, object IDs, shape area and length. The new column that differs in datatype is "geometry". 
...
**Answer**
It is a dataframe with an object id, zip codes, zipcode names, existing (I'm not sure what that means/what this column is), shape length, share area, geometry.
...
**Answer**
We see the state of maryland, broken down by zip code, with lat/long coordinates. 
...
**Answer**
It is a shapefile with a geometry column. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
We filtered for the state of Maryland. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We see Maryland divided by counties. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
The boundaries are now by zip and county. It's interesting the you can make out the chesapeake bay watershed without shapefiles for that, but I suppose this makes sense with the other shapefiles. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
A dataframe showing the number of foreclosures per zipcode in March of 2023
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
Disclaimer: I used Chat GPT to help with error messages on the "join_by" function: https://chat.openai.com/share/c5884439-5b4c-4c73-b03f-49fbfd76f97c

We are seeing another data frame with geometry. 

```{r}
#zip_codes_with_foreclosures <- md_zips |> left_join(foreclosure_zip_march_2023, join_by(ZIPCODE1=zip))

zip_codes_with_foreclosures <- md_zips %>%
  left_join(foreclosure_zip_march_2023, by = c("ZIPCODE1" = "zip"))

zip_codes_with_foreclosures
```
...
**Answer**
We are seeing a choropleth map of the foreclosure data by zip codes. 
