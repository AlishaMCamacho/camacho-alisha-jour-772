...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans 

Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean.

**Answer**

1) row 18, the Tree House Child Advocacy Center.. was changed into all caps in the grantee_clean column

2) row 29, Melwood Horticultural Training Center, Inc, was changed by removing "Inc" in the grantee_clean column 
...
1) Washington Adventist University went from all caps to "Washington Aventist University" which doesn't make sense to me ... otherwise I am not seeing any other changes? What I am seeing also differs from the textbook... for example, I am not seeing Walden Sierra entries. 
...
**Answer**
From what I understand, n_gram_merge() is differs from key_collision_merge by including slight spelling differences. However, I am not seeing a difference in how the data is presented after using both of the functions. 
