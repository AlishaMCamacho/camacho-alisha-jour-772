...
#1. calls the dataframe we created from the .csv, and we are creating a new dataframe to account for the month

baltco_911_calls_by_month <- baltco_911_calls |>

#2. creates a new column "month" by mutating the "date" column, and specifying that we only want to pull the month, and that we want to label it via its name as characters, not the numerical values assigned. So January, instead of "1."

  mutate(month = month(date, label=TRUE)) |> 

#3 alls the rows are grouped together by month

  group_by(month) |> 

#4 the function summarize adds another column, labeled "total_calls", and counts the total number of calls by month

  summarize(total_calls = n()) |> 
  
#5 The rows are arranged in descending order by the total number of calls 
...
a bar chart showing the counts by month, displayed in the calendar year order
...
**Answer**
we reordered the bar chart to show the number of calls per month from lowest to highest, as opposed to following the calendar year. 
...
**Answer**
the function coord_flip() flipped the chart from a vertical bar chart to a horizontal one. 
...
**Answer**
The background is no longer grayed out.
...
**Answer**
The background is blue, and the text appears to have lost all padding. It doesn't look great. 
...
**Answer**
We added proper labels, a title, and credit the source. The padding is still non existent, though. I suppose it fulfills the purpose of showing the findings internally, but I would not publish this externally. 
...
**Answer**
we have the total calls by date in Baltimore County
...
**Answer**
A heart beat/pulse. Or to answer this question properly, a line chart. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
the background grid has become more detailed. the week labels are all jumbled together. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
the angle of the labels changed. still not readable though. 
